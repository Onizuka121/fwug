...
export default function Home() {
  ...
  // Effect to manage initial page load
  useEffect(() => {
    const handleLoadComplete = () => setIsLoading(false);

    // If page is fully loaded, skip waiting for 'load' event
  ...
  // Effect to reset loading state on route changes
  useEffect(() => {
    setIsLoading(true);
    // Simulate a short delay for route change
  ...
  // Duplicating the array for a continuous effect
  const fullBanners = [...banners, ...banners];

  if (isLoading) {
    return (
      <div>
        <Loader /> {/* Ensure this component exists */}
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex flex-col items-center gap-6 lg:grid-cols-2 lg:gap-24 ">
              ...
              active:translate-x-0 active:translate-y-0 active:shadow-[0.05em_0.05em]`}
                  >
                    <span className="text-4xl text-[#65B1E0]">
                      Buy now $FGUW
                    </span>
                  </Link>
                  <Link
